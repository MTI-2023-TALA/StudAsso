  ...
  public async create(createAssociationDto: CreateAssociationDto): Promise<AssociationDto> {
    return await this.prismaService.$transaction(async (transaction) => {
      ...
      try {
        ...
        const { id } = await this.roleRepository.createRolePresident(createdAsso.id, transaction);
        await this.associationsMemberRepository.linkUserToRole(
          createdAsso.id,
          createAssociationDto.presidentId,
          id,
          transaction
        );
        return createdAsso;
      } catch (error) {
        // TODO: care errors not managed here because doesn't enter with change to Prisma
